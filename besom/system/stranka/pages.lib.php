...
class pages extends url {
	...
	public function pagesTab () {
		...
		$quer = 'SELECT SQL_CALC_FOUND_ROWS  *, m.menu_id, t.menu_id, m.menu_rodic, u.user_id  
										FROM be_text' . $this->prefix . ' t 
											JOIN be_menu' . $this->prefix . ' m ON t.menu_id = m.menu_id 
											JOIN be_user' . $this->prefix . ' u ON t.autor_id = u.user_id ';
		...
		elseif (isset($_GET['nav'])AND $_GET['nav'] != FALSE AND ($_GET['nav'] == 'ASC' OR $_GET['nav'] == 'DESC')) {
			$quer .= ' ORDER BY menu_nazov ' . $_GET['nav'] . ' ';
		}
		...
		if ($query->num_rows != FALSE) {
			...
			<table class="table table-hover">
				<tr>
					<th>#</th>
					<th>Obrazok</th>
					<th>Nazov <a title="A-Z" href="<?php echo URL_ADRESA . '?menu=' . $_GET['menu'] . '&title=DESC'; ?>">></a> <a title="Z-A" href="<?php echo URL_ADRESA . '?menu=' . $_GET['menu'] . '&title=ASC'; ?>"><</a></th>
					<!-- <th>Popis</th>  -->
					<th>Tags <a title="A-Z" href="<?php echo URL_ADRESA . '?menu=' . $_GET['menu'] . '&tags=DESC'; ?>">></a> <a title="Z-A" href="<?php echo URL_ADRESA . '?menu=' . $_GET['menu'] . '&tags=ASC'; ?>"><</a></th>
					<!-- <th>Url</th>  -->
					<th>Menu <a title="A-Z" href="<?php echo URL_ADRESA . '?menu=' . $_GET['menu'] . '&nav=DESC'; ?>">></a> <a title="Z-A" href="<?php echo URL_ADRESA . '?menu=' . $_GET['menu'] . '&nav=ASC'; ?>"><</a></th>
					<th>Datum <a title="max" href="<?php echo URL_ADRESA . '?menu=' . $_GET['menu'] . '&date=DESC'; ?>">></a> <a title="min" href="<?php echo URL_ADRESA . '?menu=' . $_GET['menu'] . '&date=ASC'; ?>"><</a></th>
					<th>Autor <a title="A-Z" href="<?php echo URL_ADRESA . '?menu=' . $_GET['menu'] . '&author=DESC'; ?>">></a> <a title="Z-A" href="<?php echo URL_ADRESA . '?menu=' . $_GET['menu'] . '&author=ASC'; ?>"><</a></th>
					<th>Views <a title="max" href="<?php echo URL_ADRESA . '?menu=' . $_GET['menu'] . '&views=DESC'; ?>">></a> <a title="min" href="<?php echo URL_ADRESA . '?menu=' . $_GET['menu'] . '&views=ASC'; ?>"><</a> </th>
					<th>Koment <a title="max" href="<?php echo URL_ADRESA . '?menu=' . $_GET['menu'] . '&comment=DESC'; ?>">></a> <a title="min" href="<?php echo URL_ADRESA . '?menu=' . $_GET['menu'] . '&comment=ASC'; ?>"><</a></th>
					<th>Paci <a title="max" href="<?php echo URL_ADRESA . '?menu=' . $_GET['menu'] . '&like=DESC'; ?>">></a> <a title="min" href="<?php echo URL_ADRESA . '?menu=' . $_GET['menu'] . '&like=ASC'; ?>"><</a></th>
					<th>Nepaci <a title="max" href="<?php echo URL_ADRESA . '?menu=' . $_GET['menu'] . '&dislike=DESC'; ?>">></a> <a title="min" href="<?php echo URL_ADRESA . '?menu=' . $_GET['menu'] . '&dislike=ASC'; ?>"><</a></th>
					<th>Publik <a title="ano" href="<?php echo URL_ADRESA . '?menu=' . $_GET['menu'] . '&public=DESC'; ?>">></a> <a title="nie" href="<?php echo URL_ADRESA . '?menu=' . $_GET['menu'] . '&public=ASC'; ?>"><</a></th>
			...
			while ($text = $query->fetch_assoc()) {
				...
				?>
				<tr class="<?php echo htmlspecialchars($colorA[$text['text_public']]); ?>">
					...
					<td><?php echo htmlspecialchars($text['menu_nazov']); ?></td>
					...
					<td>
							<?php
								if ($text['text_public'] == '1') {
									# active
									?><a href="<?php echo URL_ADRESA . '?menu=' . $_GET['menu'] . htmlspecialchars('&deactive=' . $text['text_id']);  ?>" alt="Zablokovat" title="Zablokovat"><i class="glyphicon glyphicon-remove" style="color: red"></i></a>&nbsp;<?php
								}
								else {
									?><a href="<?php echo URL_ADRESA . '?menu=' . $_GET['menu'] . htmlspecialchars('&active=' . $text['text_id']);  ?>" alt="Aktivovat" title="Aktivovat"><i class="glyphicon glyphicon-ok" style="color: green" ></i></a>&nbsp;<?php
								}

								?>
					<a href="<?php echo URL_ADRESA . '?menu=' . $_GET['menu'] . '&edit=' . htmlspecialchars($text['text_id']); ?>"><i class="glyphicon glyphicon-edit" style="color: #3d51b3; "></i></a> &nbsp; 
					<a href="<?php echo URL_ADRESA . '?menu=' . $_GET['menu'] . '&delete=' . htmlspecialchars($text['text_id']); ?>"><i class="glyphicon glyphicon-trash" style="color: red; "></i></a></td>
				</tr>
				<?php
	...
	public function pagesEdit ($id) {
		...
		if (isset($_POST['submit']) AND isset($_POST['nazov']) AND isset($_POST['sirka']) AND isset($_POST['popis']) AND isset($_POST['text']) /*AND isset($_POST['image'])*/ /*AND isset($_POST['date'])*//* AND isset($_POST['autor']) */AND isset($_POST['menu']) AND /*isset($_POST['publik']) AND*/ isset($_POST['tags']) /*AND isset($_POST['zobrazPopis']) AND isset($_POST['zobrazNazov']) AND isset($_POST['slider']) AND isset($_POST['popisSlider']) AND isset($_POST['koment'])*/ /*AND isset($_FILES["image"])*/ ) {
			...
			*//*
			...
			if (!empty($nazov) AND !empty($sirka) AND !empty($popis) AND !empty($text) AND !empty($menu) AND $album != '' /*AND !empty($publik)*/ AND !empty($tags) /*AND !empty($zobrazPopis) AND !empty($zobrazNazov) AND !empty($slider) AND !empty($popisSlider) AND !empty($koment)*/ /*AND !empty($_FILES["image"])*/ ) {
