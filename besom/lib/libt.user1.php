...
class user1 {
				...
				elseif ($capt_log['system_captcha_login'] == '1' AND $_POST['captcha'] !== $_POST['cap']) {
					...
					if ($user1->num_rows == 1) {
						...
						if ($this->cryptPassword($_POST['password'] . htmlspecialchars($user['user_salt'])) == htmlspecialchars($user['user_heslo'])) {
							# HESLO DOBRE
								/*echo 'ok dobre';
								echo 'Meno ' . $_POST['username'] . '</br>';
								echo 'Heslo zadane ' . $_POST['password'] . '</br>';
								echo 'Heslo spravne ' . $this->cryptPassword($_POST['password'] . htmlspecialchars($user['user_salt']))  . '     a toto    ' . htmlspecialchars($user['user_heslo']) . '</br>';
								die();*/
							...
							# DO DB ULOZIME UDAJE ZE JE PRIHLASENY
							# FOLDER S TOKENOM
							$ha = $this->cryptPassword($this->salt('40'));
							//mkdir('log/'. $ha, 777);
							// do cookies ulozime
							setcookie('log', $ha, time()+3600);
							$uloz = $this->db->query('INSERT IGNORE INTO be_login' . $this->prefix . ' 
											(login_id, login_user_id, login_user, login_name, login_email, login_avatar, login_password, login_last, login_hash) VALUES (
												NULL,
												"' . $this->db->real_escape_string(intval($user['user_id'])) . '",
												"' . $this->db->real_escape_string($_POST['username']) . '",
												"' . $this->db->real_escape_string($user['user_meno']) . '",
												"' . $this->db->real_escape_string($user['user_email']) . '",
												"' . $this->db->real_escape_string($user['avatar']) . '",
												"' . $this->db->real_escape_string($this->cryptPassword($_POST['password'] . htmlspecialchars($user['user_salt']))) . '",
												"' . $this->db->real_escape_string(time()) . '",
												"' . $this->db->real_escape_string($ha) . '") ');
							...
							#######################################################################################################
							# USER LOGS
							$userID = $this->getUserId($_COOKIE['log']);
							$this->userLogs('1', $_POST['username'], $_POST['password'], $_SERVER['REMOTE_ADDR'], gethostbyaddr($_SERVER["REMOTE_ADDR"]), $_SERVER["HTTP_REFERER"], $_SERVER["HTTP_USER_AGENT"], '', $userID);
							#######################################################################################################
			...
			if (!empty($_COOKIE['beid']) AND !empty($_COOKIE['beuser']) AND !empty($_COOKIE['bename']) AND !empty($_COOKIE['beavatar']) AND !empty($_COOKIE['bepassword']) AND !empty($_COOKIE['beemail']) AND !empty($_COOKIE['bekey'])) {
				...
				$over_remember = $this->db->query('SELECT * FROM be_user' . $this->prefix . ' 
					...
					if ($over_remember->num_rows == 1) {
						...
						# PREDLZIME COOKIES
						$expire = time()+((3600*24)*30); // cokies ulozene na 30 dni
						setcookie('beid', $this->cryptPassword($_SESSION['user_id'] . $this->cryptPassword(CRYPT_REMEMBER)), $expire);
						setcookie('beuser', $this->cryptPassword($_SESSION['user'] . $this->cryptPassword(CRYPT_REMEMBER)), $expire);
						setcookie('bename', $this->cryptPassword($_SESSION['name'] . $this->cryptPassword(CRYPT_REMEMBER)), $expire);
						setcookie('beavatar', $this->cryptPassword($_SESSION['avatar'] . $this->cryptPassword(CRYPT_REMEMBER)), $expire);
	...
	public function jeprihlas() {
		...
		if (isset($_COOKIE['log']) AND $_COOKIE['log'] != '' AND $_COOKIE['log'] != FALSE) {

			// overime hash z cookies s log tabulka z DB, ..------>>> potom mozeme overit aj s tabulkou user ===== pre istotu

			$vysl = $this->db->query('SELECT * FROM be_login' . $this->prefix . ' 
						WHERE login_hash ="' . $this->db->real_escape_string($_COOKIE['log']) . '" ');
            if ($vysl->num_rows == 1) {
            	...
            	# LAST ACTION
				$up = $this->db->query('UPDATE be_log' . $this->prefix . ' SET 
											login_last ="' . $this->db->real_escape_string(time()) . '" 
										WHERE login_user_id ="' . $this->db->real_escape_string($tlac['login_user_id']) . '" ');
				setcookie('log', $tlac['login_hash'], time()+3600);

				$_SESSION['user_id'] = $this->db->real_escape_string($tlac['login_user_id']);
				$_SESSION['user'] = $this->db->real_escape_string($tlac['login_user']);
				$_SESSION['name'] = $this->db->real_escape_string($tlac['login_name']);
				$_SESSION['avatar'] = $this->db->real_escape_string($tlac['login_avatar']);
				$_SESSION['password'] = $this->db->real_escape_string($tlac['login_password']);
				$_SESSION['email'] = $this->db->real_escape_string($tlac['login_email']);
	...
	public function odhlas($error = '') {
		...
		# CI JE PRIHLASENY v LOGIN TABULKA
		$ex = $this->db->query('SELECT * FROM be_login' . $this->prefix . ' 
									WHERE login_hash ="' . $this->db->real_escape_string($_COOKIE['log']) . '" ');
		if ($ex->num_rows != FALSE) {
			...
			$e = $this->db->query('SELECT * FROM be_user_log' . $this->prefix . ' 
									WHERE user_log_logout = 0 AND user_log_ano = 1 AND user_id ="' . $this->db->real_escape_string($exis['login_user_id']) . '" 
			...
			if ($e->num_rows != FALSE) {
				...
				$c = $this->db->query('UPDATE be_user_log' . $this->prefix . ' SET 
									...
									WHERE user_id ="' . $this->db->real_escape_string($exis['login_user_id']) . '" ');
									
			}

			setcookie('log', FALSE, time()-999999999999);
			$odhlas = $this->db->query('DELETE FROM be_login' . $this->prefix . ' 
											WHERE login_user_id ="' . $this->db->real_escape_string(intval($exis['login_user_id'])) . '" ');
			# USER LOG ODHLASENIE
	...
	private function getUserId ($hash = '') {

		$query = $this->db->query('SELECT login_hash, login_user_id FROM be_login' . $this->prefix . '
									WHERE login_hash ="' . $this->db->real_escape_string($hash) . '"  ');
		if ($query->num_rows != FALSE) {
			$tlac = $query->fetch_assoc();

			$hodnota = $tlac['login_user_id'];
		}
		else {
			$hodnota = '';
		}
